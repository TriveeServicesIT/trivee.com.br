<!DOCTYPE html>
<html lang="en">

<!-- Mirrored from techydevs.com/demos/themes/html/prizon-demo/prizon/index.html by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 27 Feb 2020 20:07:13 GMT -->
<head>
    <meta http-equiv="content-type" content="text/html; charset=utf-8">
    <meta name="author" content="TechyDevs">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">

    <title>Prizon | Multipurpose HTML5 Template</title>

    <!-- Favicon -->
    <link rel="icon" sizes="16x16" href="frontend/images/favicon.png">

    <!-- Google Fonts -->
    <link href="https://fonts.googleapis.com/css?family=Poppins:100,200,300,400,500,600,700,800,900&amp;display=swap" rel="stylesheet">

    <!-- inject:css -->
    <link rel="stylesheet" href="frontend/css/bootstrap.min.css">
    <link rel="stylesheet" href="frontend/css/font-awesome.min.css">
    <link rel="stylesheet" href="frontend/css/line-awesome.css">
    <link rel="stylesheet" href="frontend/css/animate.min.css">
    <link rel="stylesheet" href="frontend/css/owl.carousel.min.css">
    <link rel="stylesheet" href="frontend/css/owl.theme.default.min.css">
    <link rel="stylesheet" href="frontend/css/jquery.countdown.css">
    <link rel="stylesheet" href="frontend/css/magnific-popup.css">
    <link rel="stylesheet" href="frontend/css/jquery.fancybox.css">
    <link rel="stylesheet" href="frontend/css/daterangepicker.css">
    <link rel="stylesheet" href="frontend/css/style.css">
    <!-- end inject -->
</head>
<body>

<!-- start loading -->
<div id="loading">
    <div id="loading-center">
        <div id="loading-center-absolute">
            <div class="object" id="object_one"></div>
            <div class="object" id="object_two"></div>
            <div class="object" id="object_three"></div>
        </div>
    </div>
</div>
<!-- end loading -->

<!--======================================
        START HEADER AREA
    ======================================-->
<section class="header-menu-area">
    <div class="header-menu-fluid">
        <div class="container">
            <div class="row align-items-center menu-content">
                <div class="col-lg-3">
                    <div class="logo-box">
                        <a href="index.html" class="logo" title="Prizon"><img src="frontend/images/logo.png" alt="Prizon"></a>
                    </div>
                </div><!-- end col-lg-3 -->
                <div class="col-lg-9">
                    <div class="menu-wrapper">
                        <nav class="main-menu">
                            <ul>
                                <li>
                                    <a href="#">Home</a>
                                    <ul class="dropdown-menu-item">
                                        <li><a href="index.html">home - main</a></li>
                                        <li><a href="home-2.html">home - agency</a></li>
                                        <li><a href="home-3.html">home - SEO</a></li>
                                        <li><a href="home-4.html">home  - IT Service</a></li>
                                        <li><a href="home-5.html">home - Medical</a></li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="#">pages</a>
                                    <ul class="dropdown-menu-item">
                                        <li><a href="about-us.html">about us</a></li>
                                        <li><a href="services.html">services</a></li>
                                        <li><a href="service-single.html">service single</a></li>
                                        <li><a href="team.html">our team</a></li>
                                        <li><a href="team-single.html">team single</a></li>
                                        <li><a href="contact-us.html">contact us</a></li>
                                        <li><a href="page-404.html">404 page</a></li>
                                        <li><a href="page-faq.html">FAQ</a></li>
                                        <li><a href="login.html">log in</a>
                                        </li>
                                        <li><a href="sign-up.html">sign up</a>
                                        </li>
                                        <li><a href="recover.html">recover</a>
                                        </li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="#">portfolio</a>
                                    <ul class="dropdown-menu-item">
                                        <li><a href="portfolio-2.html">2 columns</a></li>
                                        <li><a href="portfolio-3.html">3 columns</a></li>
                                        <li><a href="portfolio-4.html">4 columns</a></li>
                                        <li><a href="portfolio-masonry.html">portfolio masonry</a></li>
                                        <li><a href="portfolio-full-width.html">wide version</a></li>
                                        <li><a href="portfolio-single.html">single portfolio</a></li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="#">blog</a>
                                    <ul class="dropdown-menu-item">
                                        <li><a href="blog-2.html">2 columns</a></li>
                                        <li><a href="blog-3.html">3 columns</a></li>
                                        <li><a href="blog-4.html">4 columns</a></li>
                                        <li><a href="blog-masonry.html">blog masonry</a></li>
                                        <li><a href="blog-full-width.html">wide version</a></li>
                                        <li><a href="blog-single.html">single blog</a></li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="#">shop</a>
                                    <ul class="dropdown-menu-item">
                                        <li><a href="shop-home.html">shop home</a></li>
                                        <li><a href="shop-grid.html">shop grid</a></li>
                                        <li><a href="shop-single.html">shop single</a></li>
                                        <li><a href="shopping-cart.html">Shopping Cart</a></li>
                                        <li><a href="checkout.html">Checkout</a></li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="#">elements</a>
                                    <ul class="dropdown-menu-item mega-menu">
                                        <li>
                                            <div class="mega-menu-item">
                                                <h3 class="mega-menu-item-title">Elements 1</h3>
                                                 <div class="section-divider"></div>
                                                <ul class="mega-menu-inner">
                                                    <li><a href="team-members.html"><span class="la la-users"></span>team members</a></li>
                                                    <li><a href="pricing-tables.html"><span class="la la-money"></span>pricing tables</a></li>
                                                    <li><a href="buttons.html"><span class="la la-mouse-pointer"></span>buttons</a></li>
                                                    <li><a href="icon-hover-effects.html"><span class="la la-leaf"></span>icon hover effects</a></li>
                                                    <li><a href="content-boxes.html"><span class="la la-question-circle"></span>content boxes</a></li>
                                                    <li><a href="flip-boxes.html"><span class="la la-file"></span>flip boxes</a></li>
                                                    <li><a href="alert-boxes.html"><span class="la la-warning"></span>alert boxes</a></li>
                                                    <li><a href="countdown.html"><span class="la la-clock-o"></span>countdown</a></li>
                                                </ul>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="mega-menu-item">
                                                <h3 class="mega-menu-item-title">Elements 2</h3>
                                                 <div class="section-divider"></div>
                                                <ul class="mega-menu-inner">
                                                    <li><a href="social-icons.html"><span class="la la-facebook"></span>social icons</a></li>
                                                    <li><a href="google-maps.html"><span class="la la-map"></span>google maps</a></li>
                                                    <li><a href="charts.html"><span class="la la-line-chart"></span>charts</a></li>
                                                    <li><a href="content-carousels.html"><span class="la la-sliders"></span>content carousels</a></li>
                                                    <li><a href="bullet-list.html"><span class="la la-list"></span>bullet list</a></li>
                                                    <li><a href="accordions.html"><span class="la la-plus"></span>accordions</a></li>
                                                    <li><a href="tabs.html"><span class="la la-list-alt"></span>tabs</a></li>
                                                    <li><a href="image-galleries.html"><span class="la la-image"></span>image galleries</a></li>
                                                </ul>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="mega-menu-item">
                                                <h3 class="mega-menu-item-title">Elements 3</h3>
                                                 <div class="section-divider"></div>
                                                <ul class="mega-menu-inner">
                                                    <li><a href="testimonials.html"><span class="la la-star"></span>testimonials</a></li>
                                                    <li><a href="faqs.html"><span class="la la-question"></span>faqs</a></li>
                                                    <li><a href="timeline.html"><span class="la la-hourglass"></span>timeline</a></li>
                                                    <li><a href="tooltip.html"><span class="la la-bolt"></span>tooltip</a></li>
                                                    <li><a href="modal.html"><span class="la la-columns"></span>modal</a></li>
                                                    <li><a href="heading.html"><span class="la la-h-square"></span>heading</a></li>
                                                    <li><a href="highlight-box.html"><span class="la la-bolt"></span>highlight box</a></li>
                                                    <li><a href="dual-button.html"><span class="la la-toggle-on"></span>dual button</a></li>
                                                </ul>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="mega-menu-item">
                                                <h3 class="mega-menu-item-title">Elements 4</h3>
                                                 <div class="section-divider"></div>
                                                <ul class="mega-menu-inner">
                                                    <li><a href="cards.html"><span class="la la-file-text"></span>cards</a></li>
                                                    <li><a href="info-box.html"><span class="la la-file-o"></span>info box</a></li>
                                                    <li><a href="icon-box.html"><span class="la la-tree"></span>icon box</a></li>
                                                    <li><a href="progress-bar.html"><span class="la la-tasks"></span>progress bar</a></li>
                                                    <li><a href="instagram-Widgets.html"><span class="la la-instagram"></span>Instagram Widgets</a></li>
                                                    <li><a href="video-galleries.html"><span class="la la-video-camera"></span>video galleries</a></li>
                                                    <li><a href="blockquotes.html"><span class="la la-quote-left"></span>blockquotes</a></li>
                                                    <li><a href="counters.html"><span class="la la-clock-o"></span>counters</a></li>
                                                </ul>
                                            </div>
                                        </li>
                                    </ul>
                                </li>
                                <li>
                                    <div class="search-wrap">
                                        <div class="dropdown">
                                            <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                <i class="fa fa-search"></i>
                                            </button>
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                <div class="contact-form-action">
                                                    <form method="post">
                                                        <div class="form-group">
                                                            <input class="form-control" type="search" name="search" placeholder="Search by keywords">
                                                            <button type="submit" class="theme-btn send__btn">
                                                                <i class="fa fa-search"></i>
                                                            </button>
                                                        </div><!-- end form-group -->
                                                    </form>
                                                </div><!-- end contact-form-action -->
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li>
                                    <div class="cart-wrap">
                                        <div class="dropdown">
                                            <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                <i class="fa fa-shopping-cart"></i>
                                                <span class="cart-count">2</span>
                                            </button>
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton2">
                                                <div class="cart-title">
                                                    <h4>Shopping Cart</h4>
                                                </div>
                                                <div class="cart-items">
                                                    <div class="items">
                                                        <img src="frontend/images/shop-img-small.jpg" alt="product">
                                                        <div class="item__info">
                                                            <a href="shop-single.html">Blue Round-Neck T-shirt</a>
                                                            <span class="item__info-price">$20.00</span>
                                                        </div>
                                                        <a href="#" class="item__remove">
                                                            <i class="fa fa-close"></i>
                                                        </a>
                                                    </div><!-- end items -->
                                                    <div class="items items2">
                                                        <img src="frontend/images/shop-img-small.jpg" alt="product">
                                                        <div class="item__info">
                                                            <a href="shop-single.html">Blue Round-Neck T-shirt</a>
                                                            <span class="item__info-price">$10.00</span>
                                                        </div>
                                                        <a href="#" class="item__remove">
                                                            <i class="fa fa-close"></i>
                                                        </a>
                                                    </div><!-- end items -->
                                                    <div class="cart-info">
                                                        <p>Total: <span>$30.00</span></p>
                                                        <a class="theme-btn" href="shopping-cart.html">View Cart</a>
                                                        <a class="theme-btn checkout__btn" href="checkout.html">Checkout</a>
                                                    </div><!-- end cart-info -->
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                            </ul><!-- end ul -->
                        </nav><!-- end main-menu -->
                        <div class="logo-right-button">
                            <ul>
                                <li class="button-box"><a href="#" class="theme-btn">get it now <span
                                        class="la la-caret-right"></span></a></li>
                            </ul>
                            <div class="side-menu-open">
                                <span class="menu__bar"></span>
                                <span class="menu__bar"></span>
                                <span class="menu__bar"></span>
                            </div>
                        </div><!-- end logo-right-button -->
                        <div class="side-nav-container">
                            <div class="humburger-menu">
                                <div class="humburger-menu-lines side-menu-close"></div><!-- end humburger-menu-lines -->
                            </div><!-- end humburger-menu -->
                            <div class="side-menu-wrap">
                                <ul class="side-menu-ul">
                                    <li class="sidenav__item">
                                        <a href="index.html">home</a>
                                        <span class="menu-plus-icon"></span>
                                        <ul class="side-sub-menu">
                                            <li><a href="index.html">home - main</a></li>
                                            <li><a href="home-2.html">home - agency</a></li>
                                            <li><a href="home-3.html">home - SEO</a></li>
                                            <li><a href="home-4.html">home  - IT Service</a></li>
                                            <li><a href="home-5.html">home - Medical</a></li>
                                        </ul>
                                    </li>
                                    <li class="sidenav__item">
                                        <a href="#">pages</a>
                                        <span class="menu-plus-icon"></span>
                                        <ul class="side-sub-menu">
                                            <li><a href="about-us.html">about us</a></li>
                                            <li><a href="services.html">services</a></li>
                                            <li><a href="service-single.html">service single</a></li>
                                            <li><a href="team.html">our team</a></li>
                                            <li><a href="team-single.html">team single</a></li>
                                            <li><a href="contact-us.html">contact us</a></li>
                                            <li><a href="page-404.html">404 page</a></li>
                                            <li><a href="page-faq.html">FAQ</a></li>
                                            <li><a href="login.html">log in</a>
                                            </li>
                                            <li><a href="sign-up.html">sign up</a>
                                            </li>
                                            <li><a href="recover.html">recover</a>
                                            </li>
                                        </ul>
                                    </li>
                                    <li class="sidenav__item">
                                        <a href="#">portfolio</a>
                                        <span class="menu-plus-icon"></span>
                                        <ul class="side-sub-menu">
                                            <li><a href="portfolio-2.html">2 columns</a></li>
                                            <li><a href="portfolio-3.html">3 columns</a></li>
                                            <li><a href="portfolio-4.html">4 columns</a></li>
                                            <li><a href="portfolio-masonry.html">portfolio masonry</a></li>
                                            <li><a href="portfolio-full-width.html">wide version</a></li>
                                            <li><a href="portfolio-single.html">single portfolio</a></li>
                                        </ul>
                                    </li>
                                    <li class="sidenav__item">
                                        <a href="#">blog</a>
                                        <span class="menu-plus-icon"></span>
                                        <ul class="side-sub-menu">
                                            <li><a href="blog-2.html">2 columns</a></li>
                                            <li><a href="blog-3.html">3 columns</a></li>
                                            <li><a href="blog-4.html">4 columns</a></li>
                                            <li><a href="blog-masonry.html">blog masonry</a></li>
                                            <li><a href="blog-full-width.html">wide version</a></li>
                                            <li><a href="blog-single.html">single blog</a></li>
                                        </ul>
                                    </li>
                                    <li class="sidenav__item">
                                        <a href="#">shop</a>
                                        <span class="menu-plus-icon"></span>
                                        <ul class="side-sub-menu">
                                            <li><a href="shop-home.html">shop home</a></li>
                                            <li><a href="shop-grid.html">shop grid</a></li>
                                            <li><a href="shop-single.html">shop single</a></li>
                                            <li><a href="shopping-cart.html">Shopping Cart</a></li>
                                            <li><a href="checkout.html">Checkout</a></li>
                                        </ul>
                                    </li>
                                    <li class="sidenav__item">
                                        <a href="#">elements</a>
                                        <span class="menu-plus-icon"></span>
                                        <ul class="side-sub-menu">
                                            <li><a href="team-members.html"><span class="la la-users"></span>team members</a></li>
                                            <li><a href="pricing-tables.html"><span class="la la-money"></span>pricing tables</a></li>
                                            <li><a href="buttons.html"><span class="la la-mouse-pointer"></span>buttons</a></li>
                                            <li><a href="icon-hover-effects.html"><span class="la la-leaf"></span>icon hover effects</a></li>
                                            <li><a href="content-boxes.html"><span class="la la-question-circle"></span>content boxes</a></li>
                                            <li><a href="flip-boxes.html"><span class="la la-file"></span>flip boxes</a></li>
                                            <li><a href="alert-boxes.html"><span class="la la-warning"></span>alert boxes</a></li>
                                            <li><a href="countdown.html"><span class="la la-clock-o"></span>countdown</a></li>
                                            <li><a href="social-icons.html"><span class="la la-facebook"></span>social icons</a></li>
                                            <li><a href="google-maps.html"><span class="la la-map"></span>google maps</a></li>
                                            <li><a href="charts.html"><span class="la la-line-chart"></span>charts</a></li>
                                            <li><a href="content-carousels.html"><span class="la la-sliders"></span>content carousels</a></li>
                                            <li><a href="bullet-list.html"><span class="la la-list"></span>bullet list</a></li>
                                            <li><a href="accordions.html"><span class="la la-plus"></span>accordions</a></li>
                                            <li><a href="tabs.html"><span class="la la-list-alt"></span>tabs</a></li>
                                            <li><a href="image-galleries.html"><span class="la la-image"></span>image galleries</a></li>
                                            <li><a href="testimonials.html"><span class="la la-star"></span>testimonials</a></li>
                                            <li><a href="faqs.html"><span class="la la-question"></span>faqs</a></li>
                                            <li><a href="timeline.html"><span class="la la-hourglass"></span>timeline</a></li>
                                            <li><a href="tooltip.html"><span class="la la-bolt"></span>tooltip</a></li>
                                            <li><a href="modal.html"><span class="la la-columns"></span>modal</a></li>
                                            <li><a href="heading.html"><span class="la la-h-square"></span>heading</a></li>
                                            <li><a href="highlight-box.html"><span class="la la-bolt"></span>highlight box</a></li>
                                            <li><a href="dual-button.html"><span class="la la-toggle-on"></span>dual button</a></li>
                                            <li><a href="cards.html"><span class="la la-file-text"></span>cards</a></li>
                                            <li><a href="info-box.html"><span class="la la-file-o"></span>info box</a></li>
                                            <li><a href="icon-box.html"><span class="la la-tree"></span>icon box</a></li>
                                            <li><a href="progress-bar.html"><span class="la la-tasks"></span>progress bar</a></li>
                                            <li><a href="instagram-Widgets.html"><span class="la la-instagram"></span>Instagram Widgets</a></li>
                                            <li><a href="video-galleries.html"><span class="la la-video-camera"></span>video galleries</a></li>
                                            <li><a href="blockquotes.html"><span class="la la-quote-left"></span>blockquotes</a></li>
                                            <li><a href="counters.html"><span class="la la-clock-o"></span>counters</a></li>
                                        </ul>
                                    </li>
                                </ul>
                                <div class="side-btn-box">
                                    <a href="#" class="theme-btn">get started <span class="la la-caret-right"></span></a>
                                </div>
                            </div><!-- end side-menu-wrap -->
                        </div><!-- end side-nav-container -->
                    </div><!-- end menu-wrapper -->
                </div><!-- end col-lg-9 -->
            </div><!-- end row -->
        </div><!-- end container-fluid -->
    </div><!-- end header-menu-fluid -->
</section><!-- end header-menu-area -->
<!--======================================
        END HEADER AREA
======================================-->

<!--================================
         START BANNER AREA
=================================-->
<section class="banner-area">
    <svg class="banner-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
        <path fill-opacity="1" d="M0,192L26.7,197.3C53.3,203,107,213,160,234.7C213.3,256,267,288,320,282.7C373.3,277,427,235,480,208C533.3,181,587,171,640,186.7C693.3,203,747,245,800,240C853.3,235,907,181,960,144C1013.3,107,1067,85,1120,96C1173.3,107,1227,149,1280,154.7C1333.3,160,1387,128,1413,112L1440,96L1440,0L1413.3,0C1386.7,0,1333,0,1280,0C1226.7,0,1173,0,1120,0C1066.7,0,1013,0,960,0C906.7,0,853,0,800,0C746.7,0,693,0,640,0C586.7,0,533,0,480,0C426.7,0,373,0,320,0C266.7,0,213,0,160,0C106.7,0,53,0,27,0L0,0Z"></path>
    </svg>
    <div class="banner-fluid">
        <div class="banner-item-wrap">
            <div id="perticles-js"></div>
            <div class="banner-item-table">
                <div class="banner-item-tablecell">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="banner-content text-center">
                                    <div class="section-heading">
                                        <p class="section__meta">#welcome to prizon</p>
                                        <h2 class="section__title">Meet Prizon, Your <br> Design Toolkit</h2>
                                    </div><!-- section-heading -->
                                    <div class="section-description">
                                        <p class="section__desc">
                                            Create just what you need for your Perfect Website. <br>
                                            Choose from a wide range of Elements
                                        </p>
                                    </div><!-- section-description -->
                                    <div class="btn-box">
                                        <a href="#" class="theme-btn">discover now<span class="la la-caret-right"></span></a>
                                        <a class="mfp-iframe video-play-btn btn-style-3" href="https://www.youtube.com/watch?v=5yS3Y-d5bDE">
                                            <span class="la la-play btn-icon"></span> Watch the Video
                                        </a>
                                    </div><!-- btn-box -->
                                </div><!-- banner-content -->
                            </div><!-- col-lg-12 -->
                        </div><!-- row -->
                    </div><!-- container -->
                </div><!-- banner-item-tablecell -->
            </div><!-- banner-item-table -->
        </div><!-- end banner-item-wrap -->
    </div><!-- end banner-fluid -->
</section><!-- end banner-area -->
<!--================================
        END BANNER AREA
=================================-->

<!--======================================
        START ABOUT AREA
 ======================================-->
<section class="about-area section-padding">
    <div class="circle-shape circle-shape1"></div>
    <div class="circle-shape circle-shape2"></div>
    <div class="circle-shape circle-shape3"></div>
    <div class="circle-shape circle-shape4"></div>
    <div class="container">
        <div class="row">
            <div class="col-lg-6">
                <div class="about-item">
                    <div class="about-heading section-heading">
                        <h5 class="section__meta">#about company</h5>
                        <h2 class="section__title">Welcome to the World of Prizon</h2>
                        <div class="section-divider"></div>
                    </div><!-- end section-heading -->
                    <div class="section-description">
                        <p class="section__desc">
                            Which cattle fruitful he fly visi won't let above lesser stars.
                            Fly form wonder every let third form two air seas after us
                            said day won light also together midst two female she great to open.
                        </p>
                    </div><!-- end section-description -->
                    <ul class="list-items">
                        <li>
                            <span class="la la-certificate"></span>
                            <h3>Certified Company</h3>
                            <p>
                                Medecins du Monde Jane Addams reduce child mortality challenges Ford Foundation.
                                Diversification shifting landscape advocate
                                pathway to a better life rights international.
                            </p>
                        </li>
                        <li>
                            <span class="la la-users"></span>
                            <h3>Experience Employee</h3>
                            <p>
                                Medecins du Monde Jane Addams reduce child mortality challenges Ford Foundation.
                                Diversification shifting landscape advocate
                                pathway to a better life rights international.
                            </p>
                        </li>
                    </ul>
                </div><!-- end about-item -->
            </div><!-- end col-lg-6 -->
            <div class="col-lg-6">
                <div class="image-box">
                    <img src="frontend/images/about-img.jpg" alt="about-img" class="img__item">
                    <img src="frontend/images/about-img2.jpg" alt="about-img" class="img__item">
                    <img src="frontend/images/about-img3.jpg" alt="about-img" class="img__item">
                    <img src="frontend/images/about-img4.jpg" alt="about-img" class="img__item">
                </div><!-- end image-box -->
            </div><!-- end col-lg-6 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end about-area -->
<!--======================================
        END ABOUT AREA
======================================-->

<!--======================================
        START FEATURE AREA
 ======================================-->
<section class="feature-area section--padding">
    <div class="circle-shape circle-shape1"></div>
    <div class="circle-shape circle-shape2"></div>
    <div class="circle-shape circle-shape3"></div>
    <div class="circle-shape circle-shape4"></div>
    <div class="container">
        <div class="row feature-content-wrap">
            <div class="col-lg-4 col-sm-6">
                <div class="feature-item feature-box-color">
                    <div class="hover-overlay"></div>
                    <div class="feature__icon">
                        <div class="div-strokes">
                            <span class="stroke__bar stroke__one"></span>
                            <span class="stroke__bar stroke__two"></span>
                            <span class="stroke__bar stroke__three"></span>
                        </div>
                        <i class="la la-cog"></i>
                    </div>
                    <h3 class="feature__title">Hand Craft Design</h3>
                    <p class="feature__text">Set have great you male grasses yielding yielding first their to called deep abundantly Set have great you male</p>
                    <a href="#" class="feature__btn">read more</a>
                </div><!-- end feature-item -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 col-sm-6">
                <div class="feature-item feature-box-color-2">
                    <div class="hover-overlay"></div>
                    <div class="feature__icon">
                        <div class="div-strokes">
                            <span class="stroke__bar stroke__one"></span>
                            <span class="stroke__bar stroke__two"></span>
                            <span class="stroke__bar stroke__three"></span>
                        </div>
                        <i class="la la-desktop"></i>
                    </div>
                    <h3 class="feature__title">Properly Valid Coding</h3>
                    <p class="feature__text">Set have great you male grasses yielding yielding first their to called deep abundantly Set have great you male</p>
                    <a href="#" class="feature__btn">read more</a>
                </div><!-- end feature-item -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 col-sm-6">
                <div class="feature-item feature-box-color-3">
                    <div class="hover-overlay"></div>
                    <div class="feature__icon">
                        <div class="div-strokes">
                            <span class="stroke__bar stroke__one"></span>
                            <span class="stroke__bar stroke__two"></span>
                            <span class="stroke__bar stroke__three"></span>
                        </div>
                        <i class="la la-microphone"></i>
                    </div>
                    <h3 class="feature__title">24/7 Hours Support</h3>
                    <p class="feature__text">Set have great you male grasses yielding yielding first their to called deep abundantly Set have great you male</p>
                    <a href="#" class="feature__btn">read more</a>
                </div><!-- end feature-item -->
            </div><!-- end col-lg-4 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end feature-area -->
<!--======================================
        END FEATURE AREA
======================================-->

<!--======================================
        START INFO AREA 4
======================================-->
<section class="info-box-area section-padding-bottom-2">
    <div class="container">
        <div class="row">
            <div class="col-lg-8 mx-auto">
                <div class="section-heading text-center">
                    <h5 class="section__meta">#services</h5>
                    <h2 class="section__title">What We do?</h2>
                    <div class="section-divider mx-auto"></div>
                </div><!-- end section-heading -->
                <div class="section-description m-0 text-center">
                    <p class="section__desc">
                        Which cattle fruitful he fly visi won't let above lesser stars.
                        Fly form wonder every let third form two air seas after us
                        said day won light also.
                    </p>
                </div><!-- end section-description -->
            </div><!-- end col-lg-8 -->
        </div><!-- end row -->
        <div class="row info-box-wrap">
            <div class="col-lg-4 col-sm-6">
                <div class="info-box-item info-box-item-4">
                    <div class="info-header">
                        <svg class="infobg__svg" width="180" height="180" viewBox="0 0 600 600" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(300,300)">
                                <path d="M125.6,-88.7C153,-25,158.8,35,134.2,52.8C109.7,70.7,54.8,46.3,9.7,40.8C-35.5,35.2,-71,48.3,-103.2,26.1C-135.4,3.8,-164.3,-53.8,-146.5,-111.9C-128.7,-170,-64.4,-228.5,-7.6,-224.1C49.1,-219.7,98.1,-152.3,125.6,-88.7Z" />
                            </g>
                        </svg>
                        <svg class="info__svg" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 498.752 498.752">
                                <g>
                                    <g>
                                        <path d="M480.12,435.248c-6.088-20.272-22.8-35.216-43.624-39L384,386.696v-4.144c19.632-11.4,34.008-30.848,38.488-53.84
                                            C437.032,325.704,448,312.8,448,297.376V218.36c0-22.328-14.624-41.8-35.84-48.136l-4-4
                                            c-18.608-18.608-43.344-28.848-69.656-28.848c-54.312,0-98.504,44.184-98.504,98.504v61.496c0,15.424,10.968,28.328,25.512,31.336
                                            C270,351.704,284.368,371.16,304,382.552v4.144l-52.496,9.544c-20.824,3.792-37.536,18.736-43.624,39.008l-18.632,62.128h309.504
                                            L480.12,435.248z M424,311.152V283.6c4.76,2.776,8,7.88,8,13.776S428.76,308.376,424,311.152z M264,311.152
                                            c-4.76-2.776-8-7.88-8-13.776s3.24-11,8-13.776V311.152z M264,257.376v9.136c-2.848,0.744-5.52,1.864-8,3.312V235.88
                                            c0-45.488,37.008-82.504,82.504-82.504c22.032,0,42.76,8.584,58.344,24.168l7.056,7.064l2.152,0.528
                                            c15.28,3.816,25.944,17.48,25.944,33.224v51.464c-2.48-1.448-5.152-2.576-8-3.312v-9.136h-25.368
                                            c-21.68,0-42.4-10.352-55.4-27.688l-6.36-8.496l-15.896,15.904c-13.088,13.08-30.48,20.28-48.976,20.28H264z M280,313.376v-40.368
                                            c19.752-1.832,38.08-10.4,52.288-24.6l3.112-3.112c16.04,17.704,39.144,28.08,63.232,28.08H408v40c0,35.288-28.712,64-64,64
                                            C308.712,377.376,280,348.664,280,313.376z M320.68,389.888c7.384,2.256,15.208,3.488,23.32,3.488s15.936-1.232,23.32-3.488
                                            L344,407.376L320.68,389.888z M331.824,418.24l-16.328,16.32l-10.592-31.768l5.096-0.92L331.824,418.24z M378,401.872l5.088,0.928
                                            l-10.592,31.768l-16.328-16.32L378,401.872z M210.752,481.376l12.464-41.528c4.344-14.48,16.28-25.16,31.16-27.856l34.64-6.296
                                            l19.496,58.496L344,428.688l35.496,35.496l19.496-58.496l34.64,6.296c14.872,2.704,26.816,13.384,31.16,27.856l12.456,41.536
                                            H210.752z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M208,130.728V80.024l-21.712-5.784c-0.64-1.672-1.336-3.344-2.096-5.016l11.272-19.472l-35.84-35.84l-19.472,11.272
                                            c-1.664-0.76-3.336-1.456-5-2.096l-5.808-21.712H78.656l-5.8,21.72c-1.672,0.64-3.336,1.336-5,2.096l-19.472-11.28l-35.84,35.84
                                            l11.272,19.472c-0.768,1.672-1.464,3.344-2.104,5.016L0,80.024v50.704l21.712,5.784c0.64,1.672,1.336,3.344,2.104,5.016
                                            L12.544,161l35.84,35.84l19.472-11.272c1.664,0.76,3.336,1.456,5,2.096l5.8,21.712h50.688l5.8-21.72
                                            c1.672-0.64,3.336-1.336,5-2.096l19.472,11.272l35.84-35.84l-11.272-19.472c0.76-1.672,1.464-3.344,2.096-5.016L208,130.728z
                                             M175.456,158.368l-18.464,18.472l-16.28-9.432l-3.888,1.992c-3.568,1.832-7.2,3.352-10.784,4.512l-4.152,1.336l-4.832,18.128
                                            H90.944l-4.832-18.128l-4.152-1.336c-3.584-1.168-7.216-2.68-10.784-4.512l-3.888-1.992l-16.28,9.432l-18.464-18.472l9.424-16.272
                                            l-1.992-3.888c-1.848-3.592-3.36-7.224-4.504-10.784l-1.336-4.16L16,118.432V92.32l18.136-4.832l1.336-4.16
                                            c1.144-3.552,2.656-7.184,4.504-10.784l1.992-3.888l-9.424-16.272l18.464-18.472l16.28,9.432l3.888-1.992
                                            c3.568-1.832,7.2-3.352,10.784-4.512l4.152-1.336l4.832-18.128h26.112l4.832,18.128l4.152,1.336
                                            c3.584,1.168,7.216,2.68,10.784,4.512l3.888,1.992l16.28-9.432l18.464,18.472l-9.424,16.272l1.992,3.888
                                            c1.848,3.592,3.36,7.224,4.504,10.784l1.336,4.16L192,92.32v26.112l-18.136,4.832l-1.336,4.16
                                            c-1.144,3.552-2.656,7.184-4.504,10.784l-1.992,3.888L175.456,158.368z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M148.944,138.792C156.176,129.08,160,117.528,160,105.376c0-30.872-25.12-56-56-56s-56,25.128-56,56s25.12,56,56,56
                                            c12.48,0,24.28-4.008,34.136-11.6l-9.76-12.672c-7.04,5.416-15.464,8.272-24.376,8.272c-22.056,0-40-17.944-40-40
                                            c0-22.056,17.944-40,40-40c22.056,0,40,17.944,40,40c0,8.68-2.728,16.928-7.888,23.856L148.944,138.792z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M482.344,99.72L472,110.064V81.376c0-44.112-35.888-80-80-80h-48v16h48c35.288,0,64,28.712,64,64v28.688L445.656,99.72
                                            l-11.312,11.312L464,140.688l29.656-29.656L482.344,99.72z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M40,417.376v-28.688l10.344,10.344l11.312-11.312L32,358.064L2.344,387.72l11.312,11.312L24,388.688v28.688
                                            c0,44.112,35.888,80,80,80v-16C68.712,481.376,40,452.664,40,417.376z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect y="241.376" width="208" height="16"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect y="305.376" width="208" height="16"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect y="273.376" width="96" height="16"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <polygon points="158,273.376 120,273.376 112,273.376 112,289.376 120,289.376 158,289.376 208,289.376 208,273.376"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="312" y="1.376" width="16" height="16"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="280" y="1.376" width="16" height="16"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="120" y="481.376" width="16" height="16"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="152" y="481.376" width="16" height="16"/>
                                    </g>
                                </g>
                        </svg>
                    </div>
                    <div class="info-body">
                        <h3 class="info__title"><a href="service-single.html">Web Design</a> </h3>
                        <p class="info__text">
                            Sed porttitor lectus nibh. Praesent sapien massa, convallis ndisse suscipit sagittis leo.
                        </p>
                    </div>
                </div><!-- end info-box-item info-box-item-4 -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 col-sm-6">
                <div class="info-box-item info-box-item-4">
                    <div class="info-header">
                        <svg class="infobg__svg" width="180" height="180" viewBox="0 0 600 600" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(300,300)">
                                <path d="M125.6,-88.7C153,-25,158.8,35,134.2,52.8C109.7,70.7,54.8,46.3,9.7,40.8C-35.5,35.2,-71,48.3,-103.2,26.1C-135.4,3.8,-164.3,-53.8,-146.5,-111.9C-128.7,-170,-64.4,-228.5,-7.6,-224.1C49.1,-219.7,98.1,-152.3,125.6,-88.7Z" />
                            </g>
                        </svg>
                        <svg class="info__svg" version="1.1" id="Capa_2" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 60 60">
                            <path d="M58.681,46h0.042l-0.238-1.177L52.347,8H17.5V6c0-2.206,1.794-4,4-4s4,1.794,4,4c0,0.553,0.448,1,1,1s1-0.447,1-1
                                c0-3.309-2.691-6-6-6s-6,2.691-6,6v2H7.653L1.516,44.823L1.278,46h0.042H25.5v9h-3v5h15v-5h-3v-9H58.681z M9.347,10H15.5v7
                                c0,0.553,0.448,1,1,1s1-0.447,1-1v-7h33.153l4.5,27H4.847L9.347,10z M3.722,44l1.01-5h50.536l1.01,5H34.5h-9H3.722z M35.5,57v1h-11
                                v-1h1h9H35.5z M32.5,55h-5v-9h5V55z"/>
                        </svg>
                    </div>
                    <div class="info-body">
                        <h3 class="info__title"><a href="service-single.html">Graphics Design</a> </h3>
                        <p class="info__text">
                            Sed porttitor lectus nibh. Praesent sapien massa, convallis ndisse suscipit sagittis leo.
                        </p>
                    </div>
                </div><!-- end info-box-item info-box-item-4 -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 col-sm-6">
                <div class="info-box-item info-box-item-4">
                    <div class="info-header">
                        <svg class="infobg__svg" width="180" height="180" viewBox="0 0 600 600" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(300,300)">
                                <path d="M125.6,-88.7C153,-25,158.8,35,134.2,52.8C109.7,70.7,54.8,46.3,9.7,40.8C-35.5,35.2,-71,48.3,-103.2,26.1C-135.4,3.8,-164.3,-53.8,-146.5,-111.9C-128.7,-170,-64.4,-228.5,-7.6,-224.1C49.1,-219.7,98.1,-152.3,125.6,-88.7Z" />
                            </g>
                        </svg>
                        <svg class="info__svg" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
                            <path d="m61.234 18.322a3 3 0 0 0 -4.243 0l-6.364 6.365a1 1 0 1 1 -1.414-1.415l.694-.694.013-.013 1.414-1.414a3 3 0 1 0 -4.242-4.243l-2.828 2.828a1 1 0 1 1 -1.415-1.414l2.829-2.828a3 3 0 1 0 -4.243-4.243l-2.829 2.829a1.022 1.022 0 0 1 -1.414 0 1 1 0 0 1 0-1.413l5.071-5.067a3 3 0 0 0 0-4.243 3.071 3.071 0 0 0 -4.243 0l-8.745 8.743a3.073 3.073 0 0 0 -3.4.569l-5.658 5.656a3 3 0 0 0 0 4.243l19.8 19.8a3 3 0 0 0 4.242 0l.707-.707 1.415 1.414-4.243 4.243-12.723-12.732a3.071 3.071 0 0 0 -4.243 0l-7.909 7.908a2.993 2.993 0 0 0 -3.405.577l-10.607 10.607a3 3 0 0 0 0 4.242l2.829 2.829a3 3 0 0 0 4.242 0l10.607-10.607a3 3 0 0 0 .577-3.405l5.787-5.787 12.728 12.728a3 3 0 0 0 4.242 0l8.486-8.486a3 3 0 0 0 0-4.242l-3.536-3.536.708-.707a3 3 0 0 0 .579-3.407l10.734-10.735a3 3 0 0 0 0-4.243zm-18.385 22.628a1.022 1.022 0 0 1 -1.413 0l-19.8-19.8a1 1 0 0 1 0-1.415l5.657-5.656a1 1 0 0 1 1.414 0l19.8 19.8a1 1 0 0 1 0 1.415zm-23.334 7.778-10.607 10.606a1 1 0 0 1 -1.414 0l-2.828-2.828a1 1 0 0 1 0-1.414l10.606-10.607a1 1 0 0 1 1.415 0l2.828 2.828a1 1 0 0 1 0 1.415zm31.819-6.364a1 1 0 0 1 0 1.414l-8.485 8.486a1.025 1.025 0 0 1 -1.414 0l-12.728-12.728a2 2 0 0 0 -2.828 0l-5.657 5.656-1.414-1.414 7.778-7.778a1.023 1.023 0 0 1 1.414 0l12.729 12.729a2 2 0 0 0 2.827 0l4.244-4.244a2 2 0 0 0 0-2.828l-1.414-1.414 1.414-1.415zm8.486-21.213-10.607 10.606-18.385-18.385 8.608-8.606a1.021 1.021 0 0 1 1.413 0 1 1 0 0 1 0 1.413l-5.071 5.071a3 3 0 0 0 0 4.243 3.073 3.073 0 0 0 4.243 0l2.828-2.828a1 1 0 0 1 1.415 1.414l-2.829 2.828a3 3 0 0 0 4.243 4.243l2.828-2.829a1.023 1.023 0 0 1 1.414 0 1 1 0 0 1 0 1.415l-2.12 2.122a3 3 0 0 0 4.242 4.242l6.363-6.364a1 1 0 1 1 1.415 1.414z"/>
                            <path d="m49.213 10.714a3 3 0 1 1 2.121-.877 2.988 2.988 0 0 1 -2.121.877zm0-4a1 1 0 1 0 .707 1.707 1 1 0 0 0 -.707-1.706z"/>
                            <path d="m54.87 16.373a3 3 0 0 1 -2.121-5.122 3.073 3.073 0 0 1 4.243 0 3 3 0 0 1 0 4.243 2.978 2.978 0 0 1 -2.122.879zm0-4a1 1 0 0 0 -.707 1.706 1.022 1.022 0 0 0 1.414 0 1 1 0 0 0 -.707-1.706z"/>
                        </svg>
                    </div>
                    <div class="info-body">
                        <h3 class="info__title"><a href="service-single.html">App Development</a> </h3>
                        <p class="info__text">
                            Sed porttitor lectus nibh. Praesent sapien massa, convallis ndisse suscipit sagittis leo.
                        </p>
                    </div>
                </div><!-- end info-box-item info-box-item-4 -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 col-sm-6">
                <div class="info-box-item info-box-item-4">
                    <div class="info-header">
                        <svg class="infobg__svg" width="180" height="180" viewBox="0 0 600 600" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(300,300)">
                                <path d="M125.6,-88.7C153,-25,158.8,35,134.2,52.8C109.7,70.7,54.8,46.3,9.7,40.8C-35.5,35.2,-71,48.3,-103.2,26.1C-135.4,3.8,-164.3,-53.8,-146.5,-111.9C-128.7,-170,-64.4,-228.5,-7.6,-224.1C49.1,-219.7,98.1,-152.3,125.6,-88.7Z" />
                            </g>
                        </svg>
                        <svg class="info__svg" viewBox="0 0 569 569.54905" xmlns="http://www.w3.org/2000/svg">
                            <path d="m1.527344 52.246094 37.984375 66.46875c1.28125 2.246094 3.425781 3.871094 5.933593 4.5l35.4375 8.859375 121.542969 121.542969 13.429688-13.425782-123.445313-123.441406c-1.214844-1.21875-2.738281-2.082031-4.40625-2.5l-34.050781-8.542969-32.339844-56.625 27.726563-27.726562 56.648437 32.371093 8.546875 34.050782c.414063 1.671875 1.28125 3.199218 2.496094 4.414062l123.445312 123.445313 13.425782-13.429688-121.542969-121.542969-8.859375-35.417968c-.628906-2.511719-2.253906-4.660156-4.5-5.945313l-66.472656-37.980469c-3.707032-2.109374-8.371094-1.484374-11.394532 1.527344l-37.980468 37.984375c-3.0546878 3.003907-3.71875 7.675781-1.625 11.414063zm0 0"/>
                            <path d="m396.3125 187.144531-208.902344 208.90625-13.429687-13.429687 208.90625-208.902344zm0 0"/>
                            <path d="m150.847656 403.441406c-1.71875-2.859375-4.804687-4.605468-8.140625-4.605468h-56.972656c-3.332031 0-6.421875 1.746093-8.136719 4.605468l-28.488281 47.476563c-1.808594 3.007812-1.808594 6.769531 0 9.78125l28.488281 47.476562c1.714844 2.855469 4.804688 4.605469 8.136719 4.605469h56.972656c3.335938 0 6.421875-1.75 8.140625-4.605469l28.484375-47.476562c1.808594-3.011719 1.808594-6.773438 0-9.78125zm-13.511718 90.347656h-46.226563l-22.789063-37.980468 22.789063-37.984375h46.226563l22.789062 37.984375zm0 0"/><path d="m456.0625 227.914062c62.714844.210938 113.730469-50.460937 113.941406-113.175781.03125-9.546875-1.140625-19.054687-3.488281-28.308593-1.265625-5.089844-6.417969-8.1875-11.507813-6.921876-1.671874.417969-3.195312 1.28125-4.414062 2.496094l-59.109375 59.070313-46.898437-15.628907-15.640626-46.886718 59.109376-59.121094c3.707031-3.710938 3.703124-9.722656-.007813-13.429688-1.222656-1.222656-2.761719-2.089843-4.445313-2.503906-60.820312-15.402344-122.605468 21.414063-138.007812 82.230469-2.339844 9.226563-3.507812 18.710937-3.476562 28.230469.023437 7.476562.792968 14.929687 2.308593 22.25l-207.957031 207.953125c-7.320312-1.511719-14.773438-2.28125-22.246094-2.308594-62.933594 0-113.949218 51.015625-113.949218 113.949219 0 62.929687 51.015624 113.945312 113.949218 113.945312 62.929688 0 113.945313-51.015625 113.945313-113.945312-.023438-7.476563-.796875-14.929688-2.308594-22.25l49.785156-49.785156 21.773438 21.773437c3.710937 3.707031 9.71875 3.707031 13.429687 0l4.746094-4.75c4.164062-4.136719 10.894531-4.136719 15.058594 0 4.160156 4.148437 4.167968 10.882813.019531 15.042969-.003906.003906-.011719.011718-.019531.019531l-4.746094 4.746094c-3.707031 3.707031-3.707031 9.71875 0 13.425781l113.273438 113.273438c29.792968 30.066406 78.316406 30.285156 108.382812.492187 30.0625-29.792969 30.28125-78.320313.488281-108.382813-.160156-.164062-.324219-.328124-.488281-.492187l-113.273438-113.269531c-3.707031-3.707032-9.71875-3.707032-13.425781 0l-4.746093 4.746094c-4.167969 4.140624-10.894532 4.140624-15.0625 0-4.15625-4.148438-4.167969-10.882813-.019532-15.039063.007813-.007813.015625-.011719.019532-.019531l4.75-4.75c3.707031-3.707032 3.707031-9.71875 0-13.425782l-21.773438-21.773437 49.785156-49.785156c7.320313 1.511719 14.773438 2.285156 22.246094 2.308593zm37.308594 322.851563c-6.898438-.011719-13.738282-1.257813-20.195313-3.683594l74.160157-74.164062c11.191406 29.769531-3.867188 62.972656-33.636719 74.164062-6.496094 2.441407-13.382813 3.691407-20.328125 3.683594zm-107.574219-246.792969c-10.515625 12.542969-8.867187 31.238282 3.675781 41.75 11.023438 9.238282 27.089844 9.230469 38.101563-.027344l106.5625 106.65625c1.15625 1.160157 2.238281 2.382813 3.285156 3.625l-81.1875 81.1875c-1.246094-1.042968-2.46875-2.125-3.628906-3.285156l-106.644531-106.652344c10.515624-12.542968 8.867187-31.238281-3.675782-41.75-11.023437-9.242187-27.09375-9.230468-38.105468.023438l-15.191407-15.191406 81.613281-81.492188zm38.34375-95.503906-215.410156 215.367188c-2.363281 2.359374-3.3125 5.785156-2.507813 9.023437 13.027344 51.160156-17.886718 103.195313-69.050781 116.21875-51.160156 13.027344-103.195313-17.886719-116.222656-69.050781-13.023438-51.160156 17.890625-103.195313 69.054687-116.222656 15.476563-3.9375 31.691406-3.9375 47.167969 0 3.238281.792968 6.65625-.15625 9.023437-2.503907l215.359376-215.371093c2.359374-2.359376 3.308593-5.785157 2.496093-9.019532-12.9375-50.5625 17.5625-102.039062 68.125-114.980468 9.554688-2.441407 19.4375-3.378907 29.28125-2.765626l-50.089843 50.109376c-2.542969 2.539062-3.433594 6.300781-2.296876 9.710937l18.988282 56.976563c.949218 2.832031 3.175781 5.058593 6.011718 6l56.976563 18.992187c3.40625 1.136719 7.167969.25 9.710937-2.289063l50.089844-50.089843c.113282 1.8125.171875 3.605469.171875 5.390625.265625 52.175781-41.8125 94.6875-93.988281 94.957031-8.066406.039063-16.105469-.953125-23.917969-2.953125-3.238281-.808594-6.664062.136719-9.023437 2.496094h.050781zm0 0"/>
                            <path d="m491.273438 477.578125-13.429688 13.429687-94.953125-94.953124 13.425781-13.429688zm0 0"/>
                        </svg>
                    </div>
                    <div class="info-body">
                        <h3 class="info__title"><a href="service-single.html">Social Marketing</a> </h3>
                        <p class="info__text">
                            Sed porttitor lectus nibh. Praesent sapien massa, convallis ndisse suscipit sagittis leo.
                        </p>
                    </div>
                </div><!-- end info-box-item info-box-item-4 -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 col-sm-6">
                <div class="info-box-item info-box-item-4">
                    <div class="info-header">
                        <svg class="infobg__svg" width="180" height="180" viewBox="0 0 600 600" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(300,300)">
                                <path d="M125.6,-88.7C153,-25,158.8,35,134.2,52.8C109.7,70.7,54.8,46.3,9.7,40.8C-35.5,35.2,-71,48.3,-103.2,26.1C-135.4,3.8,-164.3,-53.8,-146.5,-111.9C-128.7,-170,-64.4,-228.5,-7.6,-224.1C49.1,-219.7,98.1,-152.3,125.6,-88.7Z" />
                            </g>
                        </svg>
                        <svg class="info__svg" version="1.1" id="Capa_3" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 512 512">
                                <g>
                                    <g>
                                        <path d="M506.552,317.933c-9.739-14.312-29.351-18.1-43.707-8.452l-65.34,43.728l-0.19-136.11l22.7,21.265
                                            c4.827,4.521,11.121,7.009,17.729,7.009c0.012,0,0.026,0,0.039,0c7.134-0.01,14.022-3,18.898-8.205
                                            c4.737-5.057,7.222-11.655,6.996-18.581c-0.226-6.926-3.136-13.349-8.193-18.086l-43.035-40.314
                                            c-2.946-2.76-7.571-2.609-10.33,0.337c-2.759,2.946-2.608,7.57,0.337,10.33l43.035,40.314c2.208,2.068,3.478,4.872,3.577,7.895
                                            c0.098,3.023-0.986,5.904-3.055,8.111c-2.16,2.306-5.09,3.578-8.252,3.582c-0.006,0-0.011,0-0.017,0
                                            c-2.883,0-5.63-1.086-7.737-3.06c-0.001-0.001-0.001-0.001-0.001-0.001L259.124,67.616c-2.816-2.637-7.198-2.633-10.007,0.014
                                            L78.679,228.188c-2.201,2.074-5.079,3.16-8.103,3.076c-3.024-0.09-5.831-1.353-7.905-3.556c-2.074-2.201-3.167-5.079-3.076-8.102
                                            c0.09-3.024,1.353-5.831,3.554-7.905L246.326,39.143c4.342-4.091,11.154-4.101,15.507-0.021L369,139.514
                                            c2.945,2.759,7.57,2.609,10.33-0.337c2.759-2.946,2.608-7.57-0.337-10.33L271.827,28.455c-9.973-9.344-25.576-9.323-35.523,0.049
                                            l-58.639,55.24l-0.016-9.853c-0.004-3.106-1.218-6.024-3.417-8.216c-2.195-2.189-5.109-3.394-8.209-3.394
                                            c-0.006,0-0.011,0-0.017,0l-41.947,0.059c-3.106,0.004-6.023,1.218-8.216,3.417c-2.192,2.199-3.398,5.12-3.393,8.227l0.116,71.087
                                            l-59.439,55.993c-5.044,4.751-7.935,11.182-8.142,18.108c-0.207,6.926,2.296,13.518,7.047,18.56
                                            c4.75,5.044,11.181,7.936,18.108,8.143c0.265,0.008,0.53,0.012,0.795,0.012c6.635,0,12.918-2.49,17.767-7.06l22.649-21.336
                                            l0.09,64.291c-7.328-1.067-14.742-1.627-22.19-1.627H76.746v-4.382c0-14.711-11.969-26.68-26.68-26.68H26.68
                                            C11.969,249.092,0,261.06,0,275.772V390.29c0,4.036,3.272,7.308,7.308,7.308s7.308-3.272,7.308-7.308V275.772
                                            c0-6.652,5.412-12.064,12.064-12.064h23.385c6.652,0,12.064,5.412,12.064,12.064v11.69v149.796v11.575
                                            c0,6.652-5.412,12.064-12.064,12.064H26.68c-6.652,0-12.064-5.412-12.064-12.064v-12.788c0-4.036-3.272-7.308-7.308-7.308
                                            S0,432.009,0,436.045v12.788c0,14.711,11.969,26.68,26.68,26.68h23.385c14.711,0,26.68-11.968,26.68-26.68v-0.993l107.024,40.528
                                            c0.198,0.075,0.399,0.141,0.603,0.198c4.653,1.314,9.304,1.973,13.896,1.973c2.484,0,4.951-0.193,7.393-0.579l157.281-22.672
                                            c0.037-0.005,0.073-0.011,0.11-0.017c13.372-2.135,25.987-7.788,36.48-16.346c0.091-0.074,0.181-0.151,0.268-0.23l101.611-91.388
                                            C513.244,348.802,515.453,331.015,506.552,317.933z M163.037,76.902l0.034,20.59l-35.912,33.83l-0.089-54.371L163.037,76.902z
                                             M254.141,82.976l128.538,120.412l0.223,159.593l-20.691,13.848L340.3,375.6l-0.579-5.82
                                            c-2.039-20.498-17.171-36.987-37.18-40.922l-0.132-94.945c-0.01-7.286-5.942-13.207-13.227-13.207c-0.006,0-0.011,0-0.018,0
                                            l-69.654,0.098c-3.533,0.005-6.853,1.385-9.347,3.887c-2.495,2.501-3.866,5.825-3.861,9.357l0.116,82.781
                                            c-13.797-3.154-26.971-8.363-39.288-15.568c-1.537-0.899-3.089-1.767-4.654-2.611c-0.61-0.329-1.23-0.638-1.845-0.959
                                            c-0.952-0.497-1.902-0.997-2.864-1.474c-0.766-0.38-1.541-0.74-2.313-1.107c-0.819-0.389-1.637-0.782-2.462-1.157
                                            c-0.84-0.381-1.687-0.744-2.533-1.11c-0.773-0.334-1.546-0.67-2.324-0.991c-0.87-0.359-1.745-0.704-2.621-1.047
                                            c-0.777-0.304-1.555-0.606-2.336-0.898c-0.867-0.324-1.738-0.637-2.611-0.944c-0.819-0.289-1.639-0.571-2.463-0.846
                                            c-0.836-0.279-1.674-0.552-2.516-0.817c-0.897-0.283-1.798-0.553-2.7-0.82c-0.775-0.228-1.549-0.456-2.328-0.672
                                            c-1.017-0.283-2.039-0.548-3.062-0.81c-0.479-0.123-0.955-0.262-1.437-0.381l-0.113-80.878L254.141,82.976z M287.923,327.029
                                            l-61.125-7.037l-5.759-0.663l-0.118-83.911l66.874-0.094L287.923,327.029z M491.673,348.41l-101.511,91.298
                                            c-8.459,6.859-18.606,11.396-29.361,13.122l-157.213,22.662c-0.033,0.005-0.065,0.01-0.098,0.015l-0.109,0.017
                                            c-4.763,0.754-9.723,0.436-14.744-0.942L76.746,432.21V294.77h12.505c10.645,0,21.218,1.229,31.514,3.616
                                            c0.383,0.089,0.765,0.18,1.147,0.272c1.265,0.305,2.527,0.619,3.783,0.959c0.11,0.03,0.219,0.062,0.329,0.092
                                            c1.259,0.344,2.513,0.707,3.763,1.086c0.224,0.068,0.449,0.135,0.672,0.204c1.33,0.411,2.654,0.84,3.971,1.29
                                            c0.136,0.047,0.271,0.095,0.407,0.142c2.743,0.947,5.459,1.98,8.141,3.098c0.146,0.061,0.292,0.122,0.438,0.184
                                            c1.337,0.562,2.666,1.143,3.986,1.747c0.035,0.016,0.071,0.033,0.106,0.05c1.276,0.586,2.543,1.194,3.802,1.819
                                            c0.148,0.073,0.297,0.145,0.445,0.219c1.23,0.615,2.45,1.252,3.663,1.904c0.229,0.123,0.457,0.249,0.686,0.373
                                            c1.224,0.667,2.441,1.347,3.646,2.052c17.391,10.173,36.323,16.762,56.271,19.585c0.063,0.009,0.125,0.017,0.188,0.024
                                            l81.881,9.427c0.453,0.065,0.903,0.14,1.35,0.224c12.054,2.264,21.568,11.171,24.751,22.77c0.472,1.718,0.804,3.496,0.986,5.319
                                            l0.325,3.27l-153.46-13.445c-4.018-0.351-7.566,2.622-7.918,6.642c-0.353,4.021,2.622,7.566,6.642,7.918l162.242,14.215
                                            c0.039,0.003,0.077,0.002,0.115,0.005c0.038,0.003,0.075,0.009,0.114,0.012l30.597,1.717c0.137,0.008,0.273,0.011,0.409,0.011
                                            c0.145,0,0.288-0.023,0.432-0.032c0.163-0.01,0.326-0.016,0.488-0.036c0.218-0.028,0.431-0.075,0.645-0.122
                                            c0.153-0.034,0.307-0.059,0.457-0.103c0.234-0.067,0.459-0.158,0.686-0.249c0.122-0.049,0.248-0.086,0.368-0.142
                                            c0.341-0.158,0.673-0.34,0.989-0.552l25.972-17.381c0.001-0.001,0.002-0.002,0.003-0.003l76.702-51.332
                                            c7.717-5.187,18.251-3.152,23.482,4.536C499.249,333.181,498.062,342.736,491.673,348.41z"/>
                                    </g>
                                </g>
                        </svg>
                    </div>
                    <div class="info-body">
                        <h3 class="info__title"><a href="service-single.html">Youtube Marketing</a> </h3>
                        <p class="info__text">
                            Sed porttitor lectus nibh. Praesent sapien massa, convallis ndisse suscipit sagittis leo.
                        </p>
                    </div>
                </div><!-- end info-box-item info-box-item-4 -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 col-sm-6">
                <div class="info-box-item info-box-item-4">
                    <div class="info-header">
                        <svg class="infobg__svg" width="180" height="180" viewBox="0 0 600 600" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(300,300)">
                                <path d="M125.6,-88.7C153,-25,158.8,35,134.2,52.8C109.7,70.7,54.8,46.3,9.7,40.8C-35.5,35.2,-71,48.3,-103.2,26.1C-135.4,3.8,-164.3,-53.8,-146.5,-111.9C-128.7,-170,-64.4,-228.5,-7.6,-224.1C49.1,-219.7,98.1,-152.3,125.6,-88.7Z" />
                            </g>
                        </svg>
                        <svg class="info__svg" viewBox="0 0 569 569.54905" xmlns="http://www.w3.org/2000/svg">
                            <path d="m1.527344 52.246094 37.984375 66.46875c1.28125 2.246094 3.425781 3.871094 5.933593 4.5l35.4375 8.859375 121.542969 121.542969 13.429688-13.425782-123.445313-123.441406c-1.214844-1.21875-2.738281-2.082031-4.40625-2.5l-34.050781-8.542969-32.339844-56.625 27.726563-27.726562 56.648437 32.371093 8.546875 34.050782c.414063 1.671875 1.28125 3.199218 2.496094 4.414062l123.445312 123.445313 13.425782-13.429688-121.542969-121.542969-8.859375-35.417968c-.628906-2.511719-2.253906-4.660156-4.5-5.945313l-66.472656-37.980469c-3.707032-2.109374-8.371094-1.484374-11.394532 1.527344l-37.980468 37.984375c-3.0546878 3.003907-3.71875 7.675781-1.625 11.414063zm0 0"/>
                            <path d="m396.3125 187.144531-208.902344 208.90625-13.429687-13.429687 208.90625-208.902344zm0 0"/>
                            <path d="m150.847656 403.441406c-1.71875-2.859375-4.804687-4.605468-8.140625-4.605468h-56.972656c-3.332031 0-6.421875 1.746093-8.136719 4.605468l-28.488281 47.476563c-1.808594 3.007812-1.808594 6.769531 0 9.78125l28.488281 47.476562c1.714844 2.855469 4.804688 4.605469 8.136719 4.605469h56.972656c3.335938 0 6.421875-1.75 8.140625-4.605469l28.484375-47.476562c1.808594-3.011719 1.808594-6.773438 0-9.78125zm-13.511718 90.347656h-46.226563l-22.789063-37.980468 22.789063-37.984375h46.226563l22.789062 37.984375zm0 0"/>
                            <path d="m456.0625 227.914062c62.714844.210938 113.730469-50.460937 113.941406-113.175781.03125-9.546875-1.140625-19.054687-3.488281-28.308593-1.265625-5.089844-6.417969-8.1875-11.507813-6.921876-1.671874.417969-3.195312 1.28125-4.414062 2.496094l-59.109375 59.070313-46.898437-15.628907-15.640626-46.886718 59.109376-59.121094c3.707031-3.710938 3.703124-9.722656-.007813-13.429688-1.222656-1.222656-2.761719-2.089843-4.445313-2.503906-60.820312-15.402344-122.605468 21.414063-138.007812 82.230469-2.339844 9.226563-3.507812 18.710937-3.476562 28.230469.023437 7.476562.792968 14.929687 2.308593 22.25l-207.957031 207.953125c-7.320312-1.511719-14.773438-2.28125-22.246094-2.308594-62.933594 0-113.949218 51.015625-113.949218 113.949219 0 62.929687 51.015624 113.945312 113.949218 113.945312 62.929688 0 113.945313-51.015625 113.945313-113.945312-.023438-7.476563-.796875-14.929688-2.308594-22.25l49.785156-49.785156 21.773438 21.773437c3.710937 3.707031 9.71875 3.707031 13.429687 0l4.746094-4.75c4.164062-4.136719 10.894531-4.136719 15.058594 0 4.160156 4.148437 4.167968 10.882813.019531 15.042969-.003906.003906-.011719.011718-.019531.019531l-4.746094 4.746094c-3.707031 3.707031-3.707031 9.71875 0 13.425781l113.273438 113.273438c29.792968 30.066406 78.316406 30.285156 108.382812.492187 30.0625-29.792969 30.28125-78.320313.488281-108.382813-.160156-.164062-.324219-.328124-.488281-.492187l-113.273438-113.269531c-3.707031-3.707032-9.71875-3.707032-13.425781 0l-4.746093 4.746094c-4.167969 4.140624-10.894532 4.140624-15.0625 0-4.15625-4.148438-4.167969-10.882813-.019532-15.039063.007813-.007813.015625-.011719.019532-.019531l4.75-4.75c3.707031-3.707032 3.707031-9.71875 0-13.425782l-21.773438-21.773437 49.785156-49.785156c7.320313 1.511719 14.773438 2.285156 22.246094 2.308593zm37.308594 322.851563c-6.898438-.011719-13.738282-1.257813-20.195313-3.683594l74.160157-74.164062c11.191406 29.769531-3.867188 62.972656-33.636719 74.164062-6.496094 2.441407-13.382813 3.691407-20.328125 3.683594zm-107.574219-246.792969c-10.515625 12.542969-8.867187 31.238282 3.675781 41.75 11.023438 9.238282 27.089844 9.230469 38.101563-.027344l106.5625 106.65625c1.15625 1.160157 2.238281 2.382813 3.285156 3.625l-81.1875 81.1875c-1.246094-1.042968-2.46875-2.125-3.628906-3.285156l-106.644531-106.652344c10.515624-12.542968 8.867187-31.238281-3.675782-41.75-11.023437-9.242187-27.09375-9.230468-38.105468.023438l-15.191407-15.191406 81.613281-81.492188zm38.34375-95.503906-215.410156 215.367188c-2.363281 2.359374-3.3125 5.785156-2.507813 9.023437 13.027344 51.160156-17.886718 103.195313-69.050781 116.21875-51.160156 13.027344-103.195313-17.886719-116.222656-69.050781-13.023438-51.160156 17.890625-103.195313 69.054687-116.222656 15.476563-3.9375 31.691406-3.9375 47.167969 0 3.238281.792968 6.65625-.15625 9.023437-2.503907l215.359376-215.371093c2.359374-2.359376 3.308593-5.785157 2.496093-9.019532-12.9375-50.5625 17.5625-102.039062 68.125-114.980468 9.554688-2.441407 19.4375-3.378907 29.28125-2.765626l-50.089843 50.109376c-2.542969 2.539062-3.433594 6.300781-2.296876 9.710937l18.988282 56.976563c.949218 2.832031 3.175781 5.058593 6.011718 6l56.976563 18.992187c3.40625 1.136719 7.167969.25 9.710937-2.289063l50.089844-50.089843c.113282 1.8125.171875 3.605469.171875 5.390625.265625 52.175781-41.8125 94.6875-93.988281 94.957031-8.066406.039063-16.105469-.953125-23.917969-2.953125-3.238281-.808594-6.664062.136719-9.023437 2.496094h.050781zm0 0"/><path d="m491.273438 477.578125-13.429688 13.429687-94.953125-94.953124 13.425781-13.429688zm0 0"/>
                        </svg>
                    </div>
                    <div class="info-body">
                        <h3 class="info__title"><a href="service-single.html">Online Marketing</a></h3>
                        <p class="info__text">
                            Sed porttitor lectus nibh. Praesent sapien massa, convallis ndisse suscipit sagittis leo.
                        </p>
                    </div>
                </div><!-- end info-box-item info-box-item-4 -->
            </div><!-- end col-lg-4 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end info-area -->
<!--======================================
        END INFO AREA
======================================-->

<div class="section-block"></div>

<!--======================================
        START FUNFACT AREA
======================================-->
<section class="funfact-area section--padding text-center">
    <div class="container">
        <div class="row">
            <div class="col-lg-8 mx-auto">
                <div class="section-heading">
                    <h5 class="section__meta">#Easily Understandable & Customizable</h5>
                    <h2 class="section__title">Walkthrough Videos & Demos</h2>
                    <div class="section-divider mx-auto"></div>
                </div><!-- end section-heading -->
                <div class="section-description">
                    <p class="section__desc">
                        Which cattle fruitful he fly visi won't let above lesser stars.
                        Fly form wonder every let third form two air seas after us
                        said day won light also.
                    </p>
                </div><!-- end section-description -->
            </div><!-- end col-lg-8 -->
        </div><!-- end row -->
        <div class="row">
            <div class="col-lg-10 mx-auto">
                <div class="funfact-img-box">
                    <img src="frontend/images/funfact-img.jpg" alt="funfact-image">
                    <a class="mfp-iframe video-play-btn" href="https://www.youtube.com/watch?v=5yS3Y-d5bDE">
                        <span class="fa fa-play video__icon"></span>
                    </a>
                </div>
            </div><!-- end col-lg-10 -->
        </div><!-- end row -->
        <div class="row counter-wrap">
            <div class="col-lg-3 col-sm-6">
                <div class="counter-item counter-item-2">
                    <div class="counter-content">
                        <span class="counter__icon clip-path la la-desktop"></span>
                        <div class="counter counter__numb">945</div>
                        <h3 class="counter__title">Project Finished</h3>
                    </div>
                </div><!-- end counter-item -->
            </div><!-- end col-lg-3 col-sm-6 -->
            <div class="col-lg-3 col-sm-6">
                <div class="counter-item counter-item-2">
                    <div class="counter-content">
                        <span class="counter__icon clip-path la la-users"></span>
                        <div class="counter counter__numb">30,985</div>
                        <h3 class="counter__title">Happy Clients</h3>
                    </div>
                </div><!-- end counter-item -->
            </div><!-- end col-lg-3 col-sm-6 -->
            <div class="col-lg-3 col-sm-6">
                <div class="counter-item counter-item-2">
                    <div class="counter-content">
                        <span class="counter__icon clip-path la la-coffee"></span>
                        <div class="counter counter__numb">870</div>
                        <h3 class="counter__title">Cups of Coffee</h3>
                    </div>
                </div><!-- end counter-item -->
            </div><!-- end col-lg-3 col-sm-6 -->
            <div class="col-lg-3 col-sm-6">
                <div class="counter-item counter-item-2">
                    <div class="counter-content">
                        <span class="counter__icon clip-path la la-code"></span>
                        <div class="counter counter__numb">22,460</div>
                        <h3 class="counter__title">Lines of Code</h3>
                    </div>
                </div><!-- end counter-item -->
            </div><!-- end col-lg-3 col-sm-6 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end funfact-area -->
<!--======================================
        END FUNFACT AREA
======================================-->

<!--======================================
        START PORTFOLIO AREA
======================================-->
<section class="portfolio-area section--padding">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-8">
                <div class="section-heading">
                    <h5 class="section__meta">#portfolio</h5>
                    <h2 class="section__title">Our Latest Work</h2>
                    <div class="section-divider"></div>
                </div><!-- end section-heading -->
                <div class="section-description">
                    <p class="section__desc">
                        Which cattle fruitful he fly visi won't let above lesser stars.
                        Fly form wonder every let third form two air seas after us
                        said day won light also.
                    </p>
                </div><!-- end section-description -->
            </div><!-- end col-lg-8 -->
            <div class="col-lg-4">
                <div class="button-shared text-right">
                    <a href="portfolio-full-width.html" class="theme-btn">
                        view all work <span class="la la-caret-right"></span>
                    </a>
                </div>
            </div><!-- end col-lg-4 -->
        </div><!-- end row -->
    </div><!-- end container -->
    <div class="container-fluid">
        <div class="row portfolio-wrap">
            <div class="col-lg-3 portfolio-column">
                <div class="portfolio-item">
                    <div class="portfolio-slider">
                        <div class="portfolio-slider-item">
                            <img src="frontend/images/portfolio-img.jpg" alt="portfolio-image">
                            <div class="portfolio-overlay">
                                <a href="frontend/images/portfolio-img2.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                                    <span class="la la-plus"></span>
                                </a>
                                <a href="portfolio-single.html">
                                    <span class="la la-link"></span>
                                </a>
                            </div>
                            <div class="portfolio-desc">
                                <h3 class="portfolio__title">Console Activity</h3>
                                <p class="portfolio__meta">Illustrations, Graphics</p>
                            </div>
                        </div>
                        <div class="portfolio-slider-item">
                            <img src="frontend/images/portfolio-img2.jpg" alt="portfolio-image">
                            <div class="portfolio-overlay">
                                <a href="frontend/images/portfolio-img2.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                                    <span class="la la-plus"></span>
                                </a>
                                <a href="portfolio-single.html">
                                    <span class="la la-link"></span>
                                </a>
                            </div>
                            <div class="portfolio-desc">
                                <h3 class="portfolio__title">Console Activity</h3>
                                <p class="portfolio__meta">Illustrations, Graphics</p>
                            </div>
                        </div>
                        <div class="portfolio-slider-item">
                            <img src="frontend/images/portfolio-img3.jpg" alt="portfolio-image">
                            <div class="portfolio-overlay">
                                <a href="frontend/images/portfolio-img2.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                                    <span class="la la-plus"></span>
                                </a>
                                <a href="portfolio-single.html">
                                    <span class="la la-link"></span>
                                </a>
                            </div>
                            <div class="portfolio-desc">
                                <h3 class="portfolio__title">Console Activity</h3>
                                <p class="portfolio__meta">Illustrations, Graphics</p>
                            </div>
                        </div>
                    </div>
                </div><!-- end portfolio-item -->
            </div><!-- end col-lg-3 portfolio-column -->
            <div class="col-lg-3 portfolio-column">
                <div class="portfolio-item">
                    <img src="frontend/images/portfolio-img2.jpg" alt="portfolio-image">
                    <div class="portfolio-overlay">
                        <a href="frontend/images/portfolio-img2.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                            <span class="la la-plus"></span>
                        </a>
                        <a href="portfolio-single.html">
                            <span class="la la-link"></span>
                        </a>
                    </div>
                    <div class="portfolio-desc">
                        <h3 class="portfolio__title">Locked Steel Gate</h3>
                        <p class="portfolio__meta">Illustrations, Graphics</p>
                    </div>
                </div><!-- end portfolio-item -->
            </div><!-- end col-lg-3 portfolio-column -->
            <div class="col-lg-3 portfolio-column">
                <div class="portfolio-item">
                    <img src="frontend/images/portfolio-img3.jpg" alt="portfolio-image">
                    <div class="portfolio-overlay">
                        <a href="frontend/images/portfolio-img3.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                            <span class="la la-plus"></span>
                        </a>
                        <a href="portfolio-single.html">
                            <span class="la la-link"></span>
                        </a>
                    </div>
                    <div class="portfolio-desc">
                        <h3 class="portfolio__title">Mac Sunglasses</h3>
                        <p class="portfolio__meta">Illustrations, Graphics</p>
                    </div>
                </div><!-- end portfolio-item -->
            </div><!-- end col-lg-3 portfolio-column -->
            <div class="col-lg-3 portfolio-column">
                <div class="portfolio-item">
                    <img src="frontend/images/portfolio-img4.jpg" alt="portfolio-image">
                    <div class="portfolio-overlay">
                        <a href="frontend/images/portfolio-img4.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                            <span class="la la-plus"></span>
                        </a>
                        <a href="portfolio-single.html">
                            <span class="la la-link"></span>
                        </a>
                    </div>
                    <div class="portfolio-desc">
                        <h3 class="portfolio__title">Morning Dew</h3>
                        <p class="portfolio__meta">Illustrations, Graphics</p>
                    </div>
                </div><!-- end portfolio-item -->
            </div><!-- end col-lg-3 portfolio-column -->
            <div class="col-lg-3 portfolio-column">
                <div class="portfolio-item">
                    <img src="frontend/images/portfolio-img5.jpg" alt="portfolio-image">
                    <div class="portfolio-overlay">
                        <a href="frontend/images/portfolio-img5.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                            <span class="la la-plus"></span>
                        </a>
                        <a href="portfolio-single.html">
                            <span class="la la-link"></span>
                        </a>
                    </div>
                    <div class="portfolio-desc">
                        <h3 class="portfolio__title">Backpack Contents</h3>
                        <p class="portfolio__meta">Illustrations, Graphics</p>
                    </div>
                </div><!-- end portfolio-item -->
            </div><!-- end col-lg-3 portfolio-column -->
            <div class="col-lg-3 portfolio-column">
                <div class="portfolio-item">
                    <img src="frontend/images/portfolio-img6.jpg" alt="portfolio-image">
                    <div class="portfolio-overlay">
                        <a href="frontend/images/portfolio-img6.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                            <span class="la la-plus"></span>
                        </a>
                        <a href="portfolio-single.html">
                            <span class="la la-link"></span>
                        </a>
                    </div>
                    <div class="portfolio-desc">
                        <h3 class="portfolio__title">Sunset Bulb Glow</h3>
                        <p class="portfolio__meta">Illustrations, Graphics</p>
                    </div>
                </div><!-- end portfolio-item -->
            </div><!-- end col-lg-3 portfolio-column -->
            <div class="col-lg-3 portfolio-column">
                <div class="portfolio-item">
                    <div class="portfolio-slider">
                        <div class="portfolio-slider-item">
                            <img src="frontend/images/portfolio-img7.jpg" alt="portfolio-image">
                            <div class="portfolio-overlay">
                                <a href="frontend/images/portfolio-img7.jpg" data-fancybox="gallery"
                                   data-caption="Open Imagination">
                                    <span class="la la-plus"></span>
                                </a>
                                <a href="portfolio-single.html">
                                    <span class="la la-link"></span>
                                </a>
                            </div>
                            <div class="portfolio-desc">
                                <h3 class="portfolio__title">Console Activity</h3>
                                <p class="portfolio__meta">Illustrations, Graphics</p>
                            </div>
                        </div>
                        <div class="portfolio-slider-item">
                            <img src="frontend/images/portfolio-img2.jpg" alt="portfolio-image">
                            <div class="portfolio-overlay">
                                <a href="frontend/images/portfolio-img2.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                                    <span class="la la-plus"></span>
                                </a>
                                <a href="portfolio-single.html">
                                    <span class="la la-link"></span>
                                </a>
                            </div>
                            <div class="portfolio-desc">
                                <h3 class="portfolio__title">Console Activity</h3>
                                <p class="portfolio__meta">Illustrations, Graphics</p>
                            </div>
                        </div>
                        <div class="portfolio-slider-item">
                            <img src="frontend/images/portfolio-img.jpg" alt="portfolio-image">
                            <div class="portfolio-overlay">
                                <a href="frontend/images/portfolio-img2.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                                    <span class="la la-plus"></span>
                                </a>
                                <a href="portfolio-single.html">
                                    <span class="la la-link"></span>
                                </a>
                            </div>
                            <div class="portfolio-desc">
                                <h3 class="portfolio__title">Console Activity</h3>
                                <p class="portfolio__meta">Illustrations, Graphics</p>
                            </div>
                        </div>
                    </div>
                </div><!-- end portfolio-item -->
            </div><!-- end col-lg-3 portfolio-column -->
            <div class="col-lg-3 portfolio-column">
                <div class="portfolio-item">
                    <img src="frontend/images/portfolio-img2.jpg" alt="portfolio-image">
                    <div class="portfolio-overlay">
                        <a href="frontend/images/portfolio-img2.jpg" data-fancybox="gallery" data-caption="Open Imagination">
                            <span class="la la-plus"></span>
                        </a>
                        <a href="portfolio-single.html">
                            <span class="la la-link"></span>
                        </a>
                    </div>
                    <div class="portfolio-desc">
                        <h3 class="portfolio__title">Console Activity</h3>
                        <p class="portfolio__meta">Illustrations, Graphics</p>
                    </div>
                </div><!-- end portfolio-item -->
            </div><!-- end col-lg-3 portfolio-column -->
        </div><!-- end row -->
    </div><!-- end container-fluid -->
</section><!-- end portfolio-area -->
<!--======================================
        END PORTFOLIO AREA
======================================-->

<!--======================================
        START MOBILE AREA
======================================-->
<section class="mobile-app-area section-padding-bottom">
    <div class="container">
        <div class="row">
            <div class="col-lg-6">
                <div class="mobile-app-img">
                    <img src="frontend/images/mobile.png" alt="mobile-app">
                </div>
            </div><!-- end col-lg-6 -->
            <div class="col-lg-6">
                <div class="app-content">
                    <div class="section-heading">
                        <h5 class="section__meta">#app download</h5>
                        <h2 class="section__title">Download Our App and Make Smart Business</h2>
                        <div class="section-divider"></div>
                    </div><!-- end section-heading -->
                    <div class="section-description">
                        <p class="section__desc">
                            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aspernatur,
                            quaerat sit. Aperiam asperiores earum illo inventore laboriosam odit provident quaerat sit unde veniam.
                            Aperiam fuga maxime molestiae voluptatibus. Dignissimos, iusto.
                        </p>
                    </div><!-- end section-description -->
                    <div class="app-btn-box">
                        <h4>Install Prizon from</h4>
                        <a href="#" class="button btn-rounded btn-style-2"><span class="la la-apple btn-icon"></span> app store</a>
                        <a href="#" class="button btn-rounded btn-style-2"><span class="la la-android btn-icon"></span> google play</a>
                    </div>
                </div>
            </div><!-- end col-lg-6 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end mobile-app-area -->
<!--======================================
        END MOBILE AREA
======================================-->

<div class="section-block"></div>

<!--================================
         START TESTIMONIAL AREA
=================================-->
<section class="testimonial-area client-feedback position-relative section-padding">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="section-heading text-center">
                    <h5 class="section__meta">#testimonial</h5>
                    <h2 class="section__title">
                        Check What Our Lovely Client <br>
                        Said About us!
                    </h2>
                    <div class="section-divider mx-auto"></div>
                </div><!-- end section-heading -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
        <div class="row">
            <div class="col-lg-12">
                <div class="testimonial-item-wrap-7 testimonial-carousel">
                    <div class="testimonial-item">
                        <div class="row">
                            <div class="col-lg-5">
                                <div class="testimonial-img">
                                    <img src="frontend/images/testi-img1.jpg" alt="testimonial-image">
                                </div><!-- end testimonial__img -->
                            </div><!-- end col-lg-5 -->
                            <div class="col-lg-7">
                                <div class="testimonial-content">
                                    <div class="testimonial-desc">
                                        <span class="la la-quote-left"></span>
                                        <p class="testimonial__desc">
                                            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab accusamus accusantium adipisci aliquam aperiam aspernatur autem commodi debitis dolorum eius eligendi eveniet facere ipsum itaque iusto magnam minus molestiae natus necessitatibus neque numquam officiis, quasi qui quo repellat repellendus repudiandae sapiente sequi sit tempora tempore temporibus unde ut vel voluptates?
                                        </p>
                                    </div><!-- end testimonial-desc -->
                                    <div class="testimonial-author">
                                        <h3 class="author__title">Jack Hardson,</h3>
                                        <span class="author__meta">UI/UX Designer</span>
                                        <span class="author__rating">
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                        </span>
                                    </div><!-- end testimonial-author -->
                                </div><!-- end testimonial-content -->
                            </div><!-- end col-lg-7 -->
                        </div><!-- end row -->
                    </div><!-- end testimonial-item -->
                    <div class="testimonial-item">
                        <div class="row">
                            <div class="col-lg-5">
                                <div class="testimonial__img">
                                    <img src="frontend/images/testi-img2.jpg" alt="testimonial-image">
                                </div><!-- end testimonial__img -->
                            </div><!-- end col-lg-5 -->
                            <div class="col-lg-7">
                                <div class="testimonial-content">
                                    <div class="testimonial-desc">
                                        <span class="la la-quote-left"></span>
                                        <p class="testimonial__desc">
                                            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab accusamus accusantium adipisci aliquam aperiam aspernatur autem commodi debitis dolorum eius eligendi eveniet facere ipsum itaque iusto magnam minus molestiae natus necessitatibus neque numquam officiis, quasi qui quo repellat repellendus repudiandae sapiente sequi sit tempora tempore temporibus unde ut vel voluptates?
                                        </p>
                                    </div><!-- end testimonial-desc -->
                                    <div class="testimonial-author">
                                        <h3 class="author__title">Kavin Martin,</h3>
                                        <span class="author__meta">Manager</span>
                                        <span class="author__rating">
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                        </span>
                                    </div><!-- end testimonial-author -->
                                </div><!-- end testimonial-content -->
                            </div><!-- end col-lg-7 -->
                        </div><!-- end row -->
                    </div><!-- end testimonial-item -->
                    <div class="testimonial-item">
                        <div class="row">
                            <div class="col-lg-5">
                                <div class="testimonial__img">
                                    <img src="frontend/images/testi-img3.jpg" alt="testimonial-image">
                                </div><!-- end testimonial__img -->
                            </div><!-- end col-lg-5 -->
                            <div class="col-lg-7">
                                <div class="testimonial-content">
                                    <div class="testimonial-desc">
                                        <span class="la la-quote-left"></span>
                                        <p class="testimonial__desc">
                                            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab accusamus accusantium adipisci aliquam aperiam aspernatur autem commodi debitis dolorum eius eligendi eveniet facere ipsum itaque iusto magnam minus molestiae natus necessitatibus neque numquam officiis, quasi qui quo repellat repellendus repudiandae sapiente sequi sit tempora tempore temporibus unde ut vel voluptates?
                                        </p>
                                    </div><!-- end testimonial-desc -->
                                    <div class="testimonial-author">
                                        <h3 class="author__title">Jack Hardson,</h3>
                                        <span class="author__meta">UI/UX Designer</span>
                                        <span class="author__rating">
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                            <i class="la la-star"></i>
                                        </span>
                                    </div><!-- end testimonial-author -->
                                </div><!-- end testimonial-content -->
                            </div><!-- end col-lg-7 -->
                        </div><!-- end row -->
                    </div><!-- end testimonial-item -->
                </div><!-- end testimonial-wrap -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end testimonial-area -->
<!--================================
        END TESTIMONIAL AREA
=================================-->

<!--================================
        END TEAM AREA
=================================-->
<section class="team-area bg-color section--padding ">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="section-heading text-center">
                    <h5 class="section__meta">#expert people</h5>
                    <h2 class="section__title">Meet Our Team</h2>
                    <div class="section-divider mx-auto"></div>
                </div><!-- end section-heading -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
        <div class="row team-content-wrap">
            <div class="col-lg-3 col-sm-6">
                <div class="team-item team-item-7">
                    <div class="team-img-box">
                        <img src="frontend/images/team.jpg" alt="">
                        <ul class="team__profile">
                            <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                            <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                            <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
                            <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                        </ul>
                    </div>
                    <div class="team-titles">
                        <h3 class="team__title"><a href="team-single.html">alex smith</a></h3>
                        <p class="team__meta">Director</p>
                    </div>
                </div><!-- end team-item -->
            </div><!-- end col-lg-3 col-sm-6 -->
            <div class="col-lg-3 col-sm-6">
                <div class="team-item team-item-7">
                    <div class="team-img-box">
                        <img src="frontend/images/team2.jpg" alt="">
                        <ul class="team__profile">
                            <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                            <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                            <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
                            <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                        </ul>
                    </div>
                    <div class="team-titles">
                        <h3 class="team__title"><a href="team-single.html">Tom Carry</a></h3>
                        <p class="team__meta">Manager</p>
                    </div>
                </div><!-- end team-item -->
            </div><!-- end col-lg-3 col-sm-6 -->
            <div class="col-lg-3 col-sm-6">
                <div class="team-item team-item-7">
                    <div class="team-img-box">
                        <img src="frontend/images/team3.jpg" alt="">
                        <ul class="team__profile">
                            <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                            <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                            <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
                            <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                        </ul>
                    </div>
                    <div class="team-titles">
                        <h3 class="team__title"><a href="team-single.html">Collin Smith</a></h3>
                        <p class="team__meta">Sales</p>
                    </div>
                </div><!-- end team-item -->
            </div><!-- end col-lg-3 col-sm-6 -->
            <div class="col-lg-3 col-sm-6">
                <div class="team-item team-item-7">
                    <div class="team-img-box">
                        <img src="frontend/images/team4.jpg" alt="">
                        <ul class="team__profile">
                            <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                            <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                            <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
                            <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                        </ul>
                    </div>
                    <div class="team-titles">
                        <h3 class="team__title"><a href="team-single.html">Josh Clark</a></h3>
                        <p class="team__meta">Support</p>
                    </div>
                </div><!-- end team-item -->
            </div><!-- end col-lg-3 col-sm-6 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end team-area -->
<!--======================================
        END TEAM AREA
======================================-->

<!--======================================
        START CARD AREA
======================================-->
<section class="card-area section--padding">
    <div class="container">
        <div class="row">
            <div class="col-lg-8 mx-auto">
                <div class="section-heading text-center">
                    <h5 class="section__meta">#blog post</h5>
                    <h2 class="section__title">Latest From Blog</h2>
                    <div class="section-divider mx-auto"></div>
                </div><!-- end section-heading -->
            </div><!-- end col-lg-7 -->
        </div><!-- end row -->
        <div class="row card-wrap">
            <div class="col-lg-4 col-sm-6">
                <div class="card-item card-item-11">
                    <div class="card">
                        <div class="card-image">
                            <img src="frontend/images/img1.jpg" class="card-img-top" alt="">
                            <div class="card-info">
                                <span class="card__date"> <i class="la la-calendar"></i> Jan 8, 2019</span>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="post-meta">
                                <span class="author mr-2">
                                    <img src="frontend/images/team.jpg" alt="Techydevs"> Techydevs
                                </span>•
                                <span class="mr-2">May 15, 2018 </span> •
                                <span class="ml-2"><span class="fa fa-comments"></span> 3</span>
                            </div>
                            <h3 class="card-title"><a href="blog-single.html" class="card__link">A better workspace to boost your productivity</a></h3>
                        </div>
                    </div><!-- end card -->
                </div><!-- end card-item -->
            </div><!-- end col-lg-4 col-sm-6 -->
            <div class="col-lg-4 col-sm-6">
                <div class="card-item card-item-11">
                    <div class="card">
                        <div class="card-image">
                            <img src="frontend/images/img7.jpg" class="card-img-top" alt="">
                            <div class="card-info">
                                <span class="card__date"> <i class="la la-calendar"></i> Jan 8, 2019</span>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="post-meta">
                                <span class="author mr-2">
                                    <img src="frontend/images/team2.jpg" alt="Techydevs"> Techydevs
                                </span>•
                                <span class="mr-2">May 15, 2018 </span> •
                                <span class="ml-2"><span class="fa fa-comments"></span> 4</span>
                            </div>
                            <h3 class="card-title"><a href="blog-single.html" class="card__link">Five advanced SEO tips for doubling site traffic</a></h3>
                        </div>
                    </div><!-- end card -->
                </div><!-- end card-item -->
            </div><!-- end col-lg-4 col-sm-6 -->
            <div class="col-lg-4">
                <div class="card-item card-item-11 card-side-item">
                    <div class="card">
                        <div class="card-body">
                            <div class="post-meta">
                                <span class="author mr-2">
                                    <img src="frontend/images/team3.jpg" alt="Alex Smith"> Alex Smith
                                </span>•
                                <span class="mr-2">May 15, 2018 </span> •
                                <span class="ml-2"><span class="fa fa-comments"></span> 5</span>
                            </div>
                            <h3 class="card-title"><a href="blog-single.html" class="card__link">Three ways to prepare for running a startup</a></h3>
                        </div>
                    </div><!-- end card -->
                    <div class="card">
                        <div class="card-body">
                            <div class="post-meta">
                                <span class="author mr-2">
                                    <img src="frontend/images/team4.jpg" alt="Mark Doe"> Mark Doe
                                </span>•
                                <span class="mr-2">May 15, 2018 </span> •
                                <span class="ml-2"><span class="fa fa-comments"></span> 6</span>
                            </div>
                            <h3 class="card-title"><a href="blog-single.html" class="card__link">Three ways to prepare for running a startup</a></h3>
                        </div>
                    </div><!-- end card -->
                    <div class="card button-shared">
                        <div class="card-body">
                            <a href="blog-3.html" class="theme-btn">view all posts
                                <span class="la la-caret-right"></span>
                            </a>
                        </div>
                    </div><!-- end card -->
                </div><!-- end card-item -->
            </div><!-- end col-lg-4 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end card-area -->
<!--======================================
        END CARD AREA
======================================-->

<div class="section-block"></div>

<!-- ================================
       START CLIENTLOGO AREA
================================= -->
<section class="clientlogo-area">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="client-logo">
                    <div class="client-logo-item">
                        <img src="frontend/images/client1.svg" alt="brand image">
                    </div><!-- end client-logo-item -->
                    <div class="client-logo-item">
                        <img src="frontend/images/client2.svg" alt="brand image">
                    </div><!-- end client-logo-item -->
                    <div class="client-logo-item">
                        <img src="frontend/images/client3.svg" alt="brand image">
                    </div><!-- end client-logo-item -->
                    <div class="client-logo-item">
                        <img src="frontend/images/client4.svg" alt="brand image">
                    </div><!-- end client-logo-item -->
                    <div class="client-logo-item">
                        <img src="frontend/images/client5.png" alt="brand image">
                    </div><!-- end client-logo-item -->
                </div><!-- end client-logo -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end clientlogo-area -->
<!-- ================================
       START CLIENTLOGO AREA
================================= -->

<!--======================================
        START GET STARTED AREA
======================================-->
<section class="get-start-area">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-12">
                <div class="get-start-box">
                    <div class="col-lg-8">
                        <div class="section-heading">
                            <h5 class="section__meta">#get in touch</h5>
                            <h2 class="section__title">Do you have any projects?</h2>
                            <p class="section__sub">Contact us without any hesitate.</p>
                        </div><!-- end section-heading -->
                    </div><!-- end col-lg-8 -->
                    <div class="col-lg-4">
                        <div class="button-shared text-right">
                            <a href="#" class="theme-btn">
                                contact us <span class="la la-caret-right"></span>
                            </a>
                        </div><!-- end button-shared -->
                    </div><!-- end col-lg-4 -->
                </div><!-- end get-start-box -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end get-start-area2 -->
<!--======================================
        END GET STARTED AREA
======================================-->

<!-- ================================
       START FOOTER AREA
================================= -->
<section class="footer-area">
    <svg class="footer-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
        <path  fill-opacity="1" d="M0,64L17.1,64C34.3,64,69,64,103,53.3C137.1,43,171,21,206,21.3C240,21,274,43,309,53.3C342.9,64,377,64,411,85.3C445.7,107,480,149,514,154.7C548.6,160,583,128,617,144C651.4,160,686,224,720,213.3C754.3,203,789,117,823,117.3C857.1,117,891,203,926,218.7C960,235,994,181,1029,149.3C1062.9,117,1097,107,1131,101.3C1165.7,96,1200,96,1234,96C1268.6,96,1303,96,1337,80C1371.4,64,1406,32,1423,16L1440,0L1440,320L1422.9,320C1405.7,320,1371,320,1337,320C1302.9,320,1269,320,1234,320C1200,320,1166,320,1131,320C1097.1,320,1063,320,1029,320C994.3,320,960,320,926,320C891.4,320,857,320,823,320C788.6,320,754,320,720,320C685.7,320,651,320,617,320C582.9,320,549,320,514,320C480,320,446,320,411,320C377.1,320,343,320,309,320C274.3,320,240,320,206,320C171.4,320,137,320,103,320C68.6,320,34,320,17,320L0,320Z"></path>
    </svg>
    <div class="container">
        <div class="row">
            <div class="col-lg-3 col-sm-6">
                <div class="footer-item">
                    <div class="logo">
                        <a href="index.html" class="foot-logo"><img src="frontend/images/logo.png" alt="logo"></a>
                    </div><!-- end logo -->
                    <p class="footer__desc">
                        Morbi convallis bibendum urna ut viverra. Maecenas
                    </p>
                    <div class="footer-contact">
                        <div class="contact-item">
                            <h5 class="contact__title"><span class="la la-map-marker"></span> Headquarters:</h5>
                            <p class="contact__text">120 Orville Road Apt. 620 California, USA</p>
                        </div>
                        <div class="contact-item">
                            <h5 class="contact__title"><span class="la la-phone"></span> Contact:</h5>
                            <p class="contact__text">
                                <span class="contact__text-title">Phone:</span>
                                (91) 7547 622250
                            </p>
                            <p class="contact__text">
                                <span class="contact__text-title">Email:</span>
                                <a href="mailto:info123@prizon.com">info123@prizon.com</a>
                            </p>
                        </div>
                    </div>
                </div><!-- end footer-item -->
            </div><!-- end col-lg-3 -->
            <div class="col-lg-3 col-sm-6">
                <div class="footer-item">
                    <h4 class="footer__title">About Company</h4>
                    <div class="section-divider"></div>
                    <ul class="user-links">
                        <li><a href="#">about us</a></li>
                        <li><a href="#">portfolio</a></li>
                        <li><a href="#">team</a></li>
                        <li><a href="#">jobs</a></li>
                        <li><a href="#">contact us</a></li>
                        <li><a href="#">Help desk</a></li>
                    </ul>
                </div><!-- end footer-item -->
            </div><!-- end col-lg-3 -->
            <div class="col-lg-2 col-sm-6">
                <div class="footer-item">
                    <h4 class="footer__title">Services</h4>
                    <div class="section-divider"></div>
                    <ul class="user-links">
                        <li><a href="#">Web Development</a></li>
                        <li><a href="#">UX Design</a></li>
                        <li><a href="#">Online Marketing</a></li>
                        <li><a href="#">Social Marketing</a></li>
                        <li><a href="#">App Development</a></li>
                        <li><a href="#">Video Marketing</a></li>
                    </ul>
                </div><!-- end footer-item -->
            </div><!-- end col-lg-2 -->
            <div class="col-lg-4 col-sm-6">
                <div class="footer-item">
                    <div class="total-box">
                        <div class="total-item">
                            <h4 class="counter total__num">10,265,310</h4>
                            <p class="total__text"><span class="la la-cloud-download"></span> Total Downloads</p>
                        </div>
                        <div class="total-item">
                            <h4 class="counter total__num">26,315</h4>
                            <p class="total__text"><span class="la la-users"></span> Lovely Clients</p>
                        </div>
                    </div>
                    <div class="contact-form-action">
                        <p class="contact__text">Subscribe to Our Newsletter to get Important News, Amazing Offers & Inside Scoops:</p>
                        <form method="post">
                            <div class="form-group">
                                <i class="la la-envelope-o form-icon"></i>
                                <input class="form-control" type="email" name="email" placeholder="Enter your email">
                                <button type="submit" class="theme-btn send__btn">
                                     Subscribe
                                </button>
                            </div><!-- end form-group -->
                        </form>
                    </div><!-- end contact-form-action -->
                    <div class="footer-social">
                        <ul>
                            <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                            <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                            <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                            <li><a href="#"><i class="fa fa-youtube-play"></i></a></li>
                            <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
                        </ul>
                    </div>
                </div><!-- end footer-item -->
            </div><!-- end col-lg-4 -->
        </div><!-- end row -->
        <div class="row">
            <div class="col-lg-12">
                <div class="copy-right">
                    <p class="copy__desc">
                        &copy; Copyright Prizon 2019. Made with
                        <span class="la la-heart"></span> by <a href="https://themeforest.net/user/techydevs/portfolio">Techydevs.</a>
                    </p>
                    <ul class="condition-links">
                        <li><a href="#">Terms of Use</a></li>
                        <li><a href="#">Privacy Policy</a></li>
                    </ul>
                </div><!-- end copy-right -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end footer-area -->
<!-- ================================
       START FOOTER AREA
================================= -->

<!-- start scroll top -->
<div id="scroll-top">
    <i class="fa fa-angle-up" title="Go top"></i>
</div>
<!-- end scroll top -->



<!-- theme js files -->
<script src="frontend/js/jquery-1.10.2.min.js"></script>
<script src="frontend/js/popper.min.js"></script>
<script src="frontend/js/bootstrap.min.js"></script>
<script src="frontend/js/owl.carousel.min.js"></script>
<script src="frontend/js/magnific-popup.min.js"></script>
<script src="frontend/js/waypoint.min.js"></script>
<script src="frontend/js/jquery.counterup.min.js"></script>
<script src="frontend/js/wow.js"></script>
<script src="frontend/js/jquery.fancybox.min.js"></script>
<script src="frontend/js/isotope.js"></script>
<script src="frontend/js/progresscircle.js"></script>
<script src="frontend/js/smooth-scrolling.js"></script>
<script src="frontend/js/particles.min.js"></script>
<script src="frontend/js/particlesRun.js"></script>
<script src="frontend/js/daterangepicker.js"></script>
<script src="frontend/js/tilt.jquery.min.js"></script>
<script src="frontend/js/main.js"></script>
</body>

<!-- Mirrored from techydevs.com/demos/themes/html/prizon-demo/prizon/index.html by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 27 Feb 2020 20:08:31 GMT -->
</html>
